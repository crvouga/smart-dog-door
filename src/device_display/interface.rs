...
pub trait DeviceDisplay: Send + Sync {
    ...
    fn write_line(&mut self, line: u8, text: &str) -> Result<(), Box<dyn Error>>;
    ...
    fn set_backlight(&mut self, on: bool) -> Result<(), Box<dyn Error>>;

    /// Set the cursor position (column 0-15, row 0-1)
    fn set_cursor(&mut self, column: u8, row: u8) -> Result<(), Box<dyn Error>>;

    /// Create a custom character in CGRAM at given location (0-7)
    fn create_char(&mut self, location: u8, char_map: [u8; 8]) -> Result<(), Box<dyn Error>>;
}
